...
export function getGuestIdentity(guest: Guest): GuestIdentity {
  ...
  const name = guest.name?.trim() || `${firstName} ${lastName}`.trim() || 'Unknown'
  ...
  const fallbackKey = `${firstName}|${lastName}|${phoneNumber || ''}`.toLowerCase()
  return {
    key: fallbackKey,
    email: null,
    name,
...
export function buildGuestIdentityMap(
  events: Event[],
  guests: Guest[]
): Map<string, { identity: GuestIdentity; eventIds: Set<string>; firstSeen: string; lastSeen: string }> {
  const identityMap = new Map<
    string,
    { identity: GuestIdentity; eventIds: Set<string>; firstSeen: string; lastSeen: string }
  >()
...
export function computeLoyaltyBuckets(
  ...
  for (const { eventIds } of identityMap.values()) {
...
export function computeTopRepeatGuests(
  ...
  for (const { identity, eventIds, firstSeen, lastSeen } of identityMap.values()) {
    if (eventIds.size > 1) {
...
export function getDashboardKPIs(
  ...
  const averageGuestsPerEvent = totalEvents > 0 ? Math.round(totalGuestRows / totalEvents) : 0
  ...
  for (const event of events) {
    const eventGuestCount = guests.filter((g) => g.event_id === event.id).length
...
export function getEventKPIs(
  ...
  const earlierGuests = allGuests.filter((g) => earlierEventIds.has(g.event_id))
  ...
  const returningPercentage = uniqueGuests > 0 ? Math.round((returningGuests / uniqueGuests) * 100) : 0
